        ...
        if showPrevalence:
            ...
            for index, count in enumerate(category_counts):
                ...
                ax.text(
                    ...
                    index,
                    f"n={count}" + " ("+ {(count/sum(category_counts))*100:.1f}"+'%)',
                    va='center',
                    ha='left',
                )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate a heatmap visualization of features across different groups.'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to an AnnData object file or AnnData object containing the dataset to be visualized.',
    )
    parser.add_argument(
        '--groupBy',
        type=str,
        required=True,
        help="The key in `adata.obs` on which to group observations.",
    )
    parser.add_argument(
        '--layer',
        type=str,
        default=None,
        help="Specifies the layer of `adata` to use for the heatmap. Use 'raw' to plot raw data.",
    )
    parser.add_argument(
        '--subsetMarkers',
        type=str,
        nargs='+',
        default=None,
        help="A list of marker genes or features to include in the heatmap.",
    )
    parser.add_argument(
        '--subsetGroups',
        type=str,
        nargs='+',
        default=None,
        help="A list of group labels to include in the heatmap.",
    )
    parser.add_argument(
        '--clusterRows',
        type=bool,
        default=True,
        help="Whether to cluster rows (observations).",
    )
    parser.add_argument(
        '--clusterColumns',
        type=bool,
        default=True,
        help="Whether to cluster columns (features).",
    )
    parser.add_argument(
        '--standardScale',
        type=str,
        ...
        default=None,
        help="Normalize the data across rows or columns.",
    )
    parser.add_argument(
        '--orderRow',
        type=str,
        nargs='+',
        default=None,
        help="Custom order for the rows based on group labels.",
    )
    parser.add_argument(
        '--orderColumn',
        type=str,
        nargs='+',
        default=None,
        help="Custom order for the columns based on feature names.",
    )
    parser.add_argument(
        '--showPrevalence',
        type=bool,
        default=False,
        help="Adds a bar showing the prevalence of the feature across the groups.",
    )
    parser.add_argument(
        '--cmap', type=str, default='vlag', help="The colormap for the heatmap."
    )
    parser.add_argument(
        '--figsize',
        type=float,
        nargs=2,
        default=None,
        help="The size of the figure to create.",
    )
    parser.add_argument(
        '--saveDir',
        type=str,
        default=None,
        help="Directory to save the generated heatmap.",
    )
    parser.add_argument(
        '--fileName',
        type=str,
        default=None,
        help="Name of the file to save the heatmap.",
    )
    parser.add_argument(
        '--verbose',
        type=bool,
        default=True,
        help="If True, print additional information during execution.",
    )
